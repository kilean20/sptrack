...
int main(int argc, char *argv[])
{
    ...
    //output files
    ofstream tbt_file,raw_file,sbl_file,ebe_file,dnu_file,env_file;
    if( TBT_OUT==1 ){
        stringstream outfile1;
        outfile1<<"tbt_"<<argv[1]<<".dat";
        tbt_file.open(outfile1.str().c_str());
        tbt_file<<"#turn count sigx2 sigxp2 sigz2 sigzp2 ex ez sigxp sigzp"<<endl;
    }
    if( RAW_OUT==1 ){
        stringstream outfile2;
        outfile2<<"raw_"<<argv[1]<<".dat";
        raw_file.open(outfile2.str().c_str());
        raw_file<<scientific;
    }
    if( SBL_OUT==1 ){
        stringstream outfile3;
        outfile3<<"sbl_"<<argv[1]<<".dat";
        sbl_file.open(outfile3.str().c_str());
    }
    if( EBE_OUT==1 ){
        stringstream outfile4;
        outfile4<<"ebe_"<<argv[1]<<".dat";
        ebe_file.open(outfile4.str().c_str());
    }
    if( DNU_OUT==1 ){
        stringstream outfile5;
        outfile5<<"dnu_"<<argv[1]<<".dat";
        dnu_file.open(outfile5.str().c_str());
    }
    if( ENV_OUT==1 ){
        stringstream outfile6;
        outfile6<<"env_"<<argv[1]<<".dat";
        env_file.open(outfile6.str().c_str());
    }
...
#pragma omp parallel
    {
        int n_cpu, tid, n_thread;
        n_cpu = omp_get_num_procs();
        n_thread = omp_get_num_threads();
        tid = omp_get_thread_num();
        const double n_particle=N_particle/n_thread;
        ...
        for(j=0;j<=N_TURN;j++) {
            ...
            if(tbt_file.is_open()){
                tbt_file<<j<<" "<<count<<" "<<sigx2(0)<<" "<<sigxp2(0)<<" "<<sigz2(0)<<" "<<sigzp2(0)<<" "<<e1(0)<<" "<<e2(0)<<" "<<sigxp(0)<<" "<<sigzp(0)<<" "<<endl; //add (0) to avoid newline
            }
            if(raw_file.is_open()){
                ...
                for (unsigned int i=0; i < Ntest; i++) {
                    if(j>50)
                        raw_file<<test_x[i*8+0]<<" "<<test_x[i*8+1]<<" "<<test_x[i*8+6]<<endl;
                }
                //			raw_file<<"#n="<<j<<'F'<<endl;
                //			raw_file<<trans(X.cols(live_index))<<endl<<endl;
            }
            if(sbl_file.is_open()){
                sbl_file<<j<<" "<<STABLE.t();
                //sbl_file<<j<<" "<<trans(STABLE.elem(live_index));
            }
...
#pragma omp for
                ...
                if (FODO.Cell[k]->NAME==string("SPKICK")){
                    ...
                    if(ebe_file.is_open()){
                        ...
                        //ebe_file<<"s = "<<FODO.Cell[k]->S<<endl;
                        //ebe_file<<trans(X.rows(row0627))<<endl<<endl;
                        //ebe_file<<trans(X.submat(row0627,live_index))<<endl<<endl;
                        ebe_file<<trans(join_cols(X.rows(row06)/sqrt(betax),X.rows(row27)/sqrt(betaz)))<<endl<<endl;
                    }
                    if(env_file.is_open()){
                        if(countk%6==0){
                            sigx2  = cov(X(row0, live_index),1);
                            ...
                            sigzp -= mean(X(row2, live_index),1)*mean(X(row3, live_index),1); //this term is supposed to be zero too
                            sigzp /= sqrt(sigz2);
                            env_file <<sqrt(sigx2(0))<<" "<<sigxp(0)<<" "<<sqrt(sigz2(0))<<" "<<sigzp(0)<<endl;
                        }
                    }

                }else if (FODO.Cell[k]->NAME==string("END")){
                    if(dnu_file.is_open()){
                        dnu_file<<Dnu_x<<" "<<Dnu_z<<endl;
                    }
                    Dnu_x=0;
                    Dnu_z=0;
                }
